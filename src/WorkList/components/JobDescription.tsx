    ...
    return (
        <Paper elevation={8} style={{marginTop: 8}}>
            <Box style={{margin: 8}}>
                ...
                <Text style={style.text}>Смена: {data.timeStartByCity} - {data.timeEndByCity}</Text>
                <Text style={style.text}>Уже набрано сотрудников: {data.currentWorkers}</Text>
                <Text style={style.text}>Планируется сотрудников: {data.planWorkers}</Text>
                <Text style={style.text}>Выплата за смену: {data.priceWorker} рублей</Text>
            </Box>
        </Paper>
    )
}
